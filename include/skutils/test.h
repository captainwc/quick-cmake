...
#define RUN_DEMO(func, ...)                                                                                          \
  do {                                                                                                               \
    ...
    {                                                                                                                \
      GUARD_LOG;                                                                                                     \
      std::cout << ANSI_YELLOW_BG << sk::utils::format("DEMO[{}] BEGIN ", demoid) << ANSI_BLUE_BG << "====== FUNC [" \
                << ANSI_PURPLE_BG << #func << ANSI_BLUE_BG << "] ======" << ANSI_CLEAR << "\n";                      \
    }                                                                                                                \
    ...
    {                                                                                                                \
      GUARD_LOG;                                                                                                     \
      std::cout << ANSI_YELLOW_BG << sk::utils::format("DEMO[{}]   END ", demoid) << ANSI_BLUE_BG << "====== TIME [" \
                << ANSI_PURPLE_BG << std::chrono::duration_cast<std::chrono::microseconds>(end - start).count()      \
                << " us" << ANSI_BLUE_BG << "] ======" << ANSI_CLEAR << "\n";                                        \
    }                                                                                                                \
  } while (0)
...
#define ASSERT(expr)                                                                                              \
  do {                                                                                                            \
    if (!(expr)) {                                                                                                \
      GUARD_LOG;                                                                                                  \
      std::cerr << ANSI_RED_BG << "Assert " << ANSI_PURPLE_BG << #expr << ANSI_RED_BG << " Failed!" << ANSI_CLEAR \
                << "\n";                                                                                          \
      THREAD_SAFE_EXIT(EXIT_ASSERT_FAIL);                                                                         \
    }                                                                                                             \
  } while (0)
  ...
  do {                                                                                                                 \
    if (expr) {                                                                                                        \
      ...
      std::cout << ANSI_GREEN_BG << "[PASSED] " << ANSI_CLEAR                                                          \
                << "[" + std::to_string(sk::utils::GlobalInfo::getInstance().gTotalTest.load()) + "] " << ANSI_BLUE_BG \
                << #expr << ANSI_GREEN_BG << " => true" << ANSI_CLEAR << "\n";                                         \
    } else {                                                                                                           \
      ++sk::utils::GlobalInfo::getInstance().gFailedTest;                                                              \
      GUARD_LOG;                                                                                                       \
      std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR                                                            \
                << "[" + std::to_string(sk::utils::GlobalInfo::getInstance().gTotalTest.load()) + "] " << ANSI_BLUE_BG \
                << #expr << ANSI_RED_BG << " => istrue? " << ANSI_YELLOW_BG << COUT_POSITION << ANSI_CLEAR << "\n";    \
...
#define ASSERT_EQUAL(x, y)                                                                                             \
  do {                                                                                                                 \
    if ((x) == (y)) {                                                                                                  \
      ...
      std::cout << ANSI_GREEN_BG << "[PASSED] " << ANSI_CLEAR                                                          \
                << "[" + std::to_string(sk::utils::GlobalInfo::getInstance().gTotalTest.load()) + "] " << ANSI_BLUE_BG \
                << #x << ANSI_GREEN_BG << " == " << ANSI_BLUE_BG << #y << ANSI_CLEAR << "\n";                          \
    } else {                                                                                                           \
      ++sk::utils::GlobalInfo::getInstance().gFailedTest;                                                              \
      GUARD_LOG;                                                                                                       \
      std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR                                                            \
                << "[" + std::to_string(sk::utils::GlobalInfo::getInstance().gTotalTest.load()) + "] " << ANSI_BLUE_BG \
                << #x << ANSI_RED_BG " == " << ANSI_BLUE_BG << #y " " << ANSI_YELLOW_BG << COUT_POSITION << ANSI_CLEAR \
                << "\n";                                                                                               \
...
#define STR_EQUAL_(x, y)                                                                                               \
  do {                                                                                                                 \
    ...
    if (expected == actual) {                                                                                          \
      ...
      std::cerr << ANSI_RED_BG << "[FAILED] " << ANSI_CLEAR                                                            \
                << "[" + std::to_string(sk::utils::GlobalInfo::getInstance().gTotalTest.load()) + "] " << ANSI_BLUE_BG \
                << #x << ANSI_RED_BG << " == " << ANSI_BLUE_BG << #y " " << ANSI_YELLOW_BG << COUT_POSITION            \
                << ANSI_CLEAR << "\n";                                                                                 \
